...
def make_prediction(species, ssp, year):
    raster_features = sorted(glob.glob('data/MIROC6/' + ssp + '/' + str(year) + '/*.tif'))
...
for species in tqdm(species_list, desc="Species"):
        for ssp in tqdm(ssp_list, desc="ssp"):
            for year in tqdm(year_list, desc="Years"):
                ...
                rds = rioxarray.open_rasterio('outputs/tiff-images/' + species + '/' + ssp + '/' + str(year) + '/probability_1.0.tif')
                rds_3587 = rds.rio.reproject("EPSG:3587")
                rds_3587.rio.to_raster('outputs/tiff-images/' + species + '/' + ssp + '/' + str(year) + '/3587/probability_1.0.tif')
