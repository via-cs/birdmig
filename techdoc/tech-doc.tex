...
\subsubsection*{React}
ReactJS was employed to construct the frontend, with the primary component located at \texttt{src/App.js}. Within \texttt{App.js}, backend calls are made to gather data for various components situated in the \texttt{src/components} directory. Among these components, \texttt{BirdInfo.js} returns a summary of the desired bird. Additionally, there are four graphs: \texttt{ClimateChart.js}, \texttt{HeatMap.js}, \texttt{PolylineMap.js}, and \texttt{SDMchart.js}. These files contain the code for displaying the average temperature and prediction graph, the trajectory graphs discussed further in the Leaflet subsection, and the PNG of the Species Distribution Model (SDM) output, respectively.

Also within the \texttt{components} folder is \texttt{PredictionControl.js}, which contains the code for displaying the year slider and SSP buttons that alter the Climate and SDM Charts. Besides the components, the sidebar and header are created in \texttt{App.js}, allowing users to view different birds. All styling is managed within \texttt{src/App.css}.
